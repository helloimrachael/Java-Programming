...
public class MyDataBoard<E extends Data> implements DataBoard<E> {
    ...
    // AF: <passwordBacheca, {category(0)...category(n)}, {dato(0)...dato(n)}, {arraycoppie(0)...arraycoppie(n)}> 
        // dove arraycoppie(i) = <friend(i), categories(i)> con 0 <= i < n.
    // RI: password != null && categoryBacheca != null && dataList != null && friends != null
    //     for all i in categoryBacheca --> categoryBacheca.get(i) != null con 0 <= i < categoryBacheca.size() &&
    //     for all j in dataList --> dataList.get(j) != null con 0 <= j < dataList.size() &&
    //     for all k in friendss --> friends.get(k) != null con 0 <= k < friends.size()
    ...
    public void insertLike(String friend, E data)
        ...
        int i = 0;
        while (trovato == false && i < dataList.size()) {
            if (dataList.get(i).getTitolo().equals(data.getTitolo()) == true) // Controllo se è presente il dato nella lista.
                trovato = true;
            else
                i++;
        }
        if (trovato == true) {
            ...
            while (trovata == false && j < friends.size()) { // Controllo se l'amico è nella lista di amici.
                if (friends.get(j).getName().equals(friend) == true) {
                    trovata = true;
                }
                else
                    j++;
            ...
            if (trovata == false)
                throw new FriendNotFoundException(friend + " non è nella lista di amici della bacheca");
            if (trovata == true) {
                if (friends.get(j).checkCatecory(c) == true) { // Aggiungo il like al brano.
    ...
    public Iterator<E> getFriendIterator(String friend) throws NullPointerException, FriendNotFoundException {
        ...
        int j;
        for (i = 0; i < allCategoryFriends.size(); i++) { // Scorre la lista delle categorie visibili a friend.
